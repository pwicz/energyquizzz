    ...
    //CHECKSTYLE:ON
        ...
        public void processAnswer(ClientMessage msg, Player p, Game g) {
            ...
            if(g.getType() == Question.Type.ESTIMATION){
                //correct if answer is +/- 20%
                if(msg.chosenActivity >= g.getCorrectAnswerID()*0.5 &&
                        msg.chosenActivity <= g.getCorrectAnswerID()*1.5){
                    long base = g.getCorrectAnswerID() - (long) (g.getCorrectAnswerID()*0.5);
                    long off = Math.abs(g.getCorrectAnswerID() - msg.chosenActivity);
                    double diff = (double) off / base;
                    scoreForQuestion = scoreBase + (int)(100 * (1 -  diff));
